...
import { ShieldCheck, ArrowLeft, Smartphone, Mail, ArrowRight } from 'lucide-react';
...
export default function VerifyPage() {
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    if (pastedData.length >= 6) {
      ...
      setOtp(digits);
      toast.success('6-digit code pasted!');
  ...
  const handleVerify = async (code = otp.join('')) => {
    ...
    try {
      ...
      toast.error(err?.message || 'Invalid or expired verification code. Please check your messages.');
    } finally {
    ...
    <div className="w-full max-w-md mx-auto p-6 md:p-8 bg-white rounded-3xl shadow-sm border border-slate-200 text-center sm:text-left my-8">
      ...
      <div className="space-y-6">
        ...
        >
          {otp.map((digit, index) => (
            <input
              ...
              className="w-11 h-14 sm:w-12 sm:h-16 text-center text-2xl font-bold text-slate-900 border-2 border-slate-200 rounded-2xl focus:border-teal-500 focus:ring-0 focus:outline-none transition-colors font-mono placeholder:text-slate-300"
            />
          ))}
        </div>
        ...
        <button
          ...
          className="w-full py-3.5 px-4 rounded-xl shadow-md text-sm font-bold text-white bg-teal-600 hover:bg-teal-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all cursor-pointer flex items-center justify-center gap-2"
        >
          {isVerifying ? (
        ...
        <div className="pt-2 border-t border-slate-100 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="py-2 px-2.5 rounded-xl border border-slate-200 hover:border-teal-400 hover:bg-slate-50 text-slate-700 text-xs font-semibold transition-all flex items-center justify-center gap-1.5 disabled:opacity-50 cursor-pointer"
            >
