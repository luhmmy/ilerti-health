...
export function GoogleSignInButton({
  ...
  // Primary action: direct browser redirect to Google OAuth
  const handleDirectGoogleRedirect = () => {
    setLoading(true);
    const targetRole = isDoctor ? "doctor" : role;
    const returnUrl = isDoctor ? "/doctor-portal" : "/dashboard";
    
    // Redirect browser to Google OAuth redirect handler
    window.location.href = `/api/auth/google/redirect?role=${encodeURIComponent(targetRole)}&returnUrl=${encodeURIComponent(returnUrl)}`;
  };

  const handleManualGoogleAuth = async (email: string, name: string) => {
    if (!email.trim()) {
    ...
    try {
      ...
      toast.success(`Google authorization verified! Welcome, ${user.name}.`);
  ...
  return (
    <>
      <button
        ...
        disabled={loading}
        className="w-full flex items-center justify-center gap-3 py-2.5 px-4 bg-white hover:bg-slate-50 border border-slate-200 hover:border-slate-300 text-slate-700 font-semibold text-sm rounded-xl shadow-xs transition-all cursor-pointer disabled:opacity-60 group"
      >
      ...
      {/* Manual Quick Google Login Modal (fallback) */}
      {showManualModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in duration-150">
          <div className="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-slate-100 relative text-left">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-slate-900 text-base font-heading">Google Authentication</h3>
                <p className="text-xs text-slate-500">Sign in with your Google Workspace or Gmail account</p>
              </div>
            </div>
            ...
            <div className="space-y-3 pt-2">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  type="button"
                  onClick={() => handleManualGoogleAuth(customEmail, customName)}
